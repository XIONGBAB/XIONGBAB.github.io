...
$(function () {
    // 节流阀
    let flag = true;
    $(".left_navbar>ul>li").click(function () {
        flag = false;
...
window.addEventListener("load", function () {
    ...
    (function renderData() { // 渲染information 数据到html页面函数
        let rightLink = document.querySelector(".main_container");
        let seriesArr = [];
        data.forEach(e => {
    ...
    function animationType() { // 标签类型动画函数
        ...
        for (let i = 0; i < linkInfos.length; i++) {
            linkInfos[i].addEventListener("mouseover", function () {
                var moneyBoxs = linkInfos[i].querySelectorAll(".hot_box em");
